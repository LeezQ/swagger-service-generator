...
async function run() {
  ...
  const { url, outDir, request = `import request from 'umi-request';`, filePathReg = '/(.*?)/', fileNameReg } = config;
  ...
  _.map(pathGroups, (pathGroup, groupKey) => {
    const fileName = (fileNameReg ? fileNameReg.replace(/\$1/g, groupKey) : groupKey) + '.ts';
